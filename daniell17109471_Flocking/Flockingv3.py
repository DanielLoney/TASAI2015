...
label = myfont.render("Mouse click to spawn boids. Space to toggle personal space. Press c to clear boids. Escape to exit", 1, (255,0,0))
...
class boid:
    ...
    def mousedirection(self):
        return self.pointdirection(pygame.mouse.get_pos())
    def seperation(self):
        ...
        avgx = avgx/objectlen
        avgy = avgy/objectlen
        if self.pointdirection((avgx, avgy))==False:#if it's to its right
            return True #Turn Left
        else:
            return False#Turn Right
    def setRotspeed(self):
        isleft = self.mousedirection()
        if self.seperation() != None:
            if self.seperation() == True:
